...
function renderExtra(prop) {
  ...
  switch (type.name) {
    ...
    case "objectOf":
      ...
      return null;
    case "signature":
      return renderSignature(prop);
    case "Array":
      return <div>{type.raw}</div>;
    default:
      return null;
  }
}

function renderSignature(prop) {
  return <div>{getType(prop).raw}</div>;
}

function renderUnion(prop) {
  if (!Array.isArray(getType(prop).elements)) {
    return <span>{getType(prop).elements}</span>;
  }

  const values = getType(prop).elements.map(element =>
    <Type key={element.name + "-" + element.value}>{renderType(element)}</Type>
  );
...
// function renderUnion(prop) {
//   console.log('getType(prop)', getType(prop))
//   if (!Array.isArray(getType(prop).value)) {
//     return <span>{getType(prop).value}</span>;
//   }
//
